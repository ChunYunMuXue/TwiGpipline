...
class LongCondenserConfig:
    ...
    # MLP hidden ratio
    mlp_ratio: float = 2.0
    # 分块大小：越大越快但显存更高；越小越省显存但更慢
    chunk_size: int = 256
    # 是否用 float32 做 streaming accumulator（更稳，但略慢）
...
class LongAttentionCondenser(nn.Module):
    """
    输入:
      - h_img_seq: [B, S, D]  (S 可以很大：前置生成的所有 image-token hidden)
    ...
    实现：
      - M 个可学习 latents 作为 query，对长序列做 cross-attention
      - 采用 chunk + streaming softmax，避免一次性构建 [B,H,M,S] 的巨大 attention logits
    """
